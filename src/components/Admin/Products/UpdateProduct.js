...
export default function UpdateProduct() {
  ...
  */
  ...
  const handleFileChange = (eventObj) => {
    const newFiles = Array.from(eventObj.target.files);
  ...
  */
  ...
  // TODO: get product from store
  const { loading, product, isUpdated, error } = useSelector((state) => {
    return state?.products;
  });
  ...
  setTimeout(() => {
    if (product?.product?._id.toString() === id.toString()) {
      setFormData({
        name: product?.product?.name,
        description: product?.product?.description,
        category: product?.product?.category,
              ...
              <div>
                ...
                <div className="mt-1">
                  <textarea
                    ...
                    onChange={handleOnChange}
                    className="block p-3 w-full rounded-md border-gray-300 border shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                  />
                </div>
              </div>
              <div>
